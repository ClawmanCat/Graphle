...
namespace graphle::test {
    ...
    /**
     * Return a view of the range contained in the provided map at the given key,
     * or an empty view of the same type if the map contains no such key.
     */
    template <typename M, typename K> constexpr auto range_at_map_key(M&& map, const K& key) {
        using view_type = decltype(std::views::all(map.at(key)));

        return map.contains(key)
            ? views::maybe_exists(std::views::all(map.at(key)))
            : views::maybe_exists(meta::deduce_as<view_type>);
    }
    ...
    struct ve_map_graph {
        ...
        constexpr auto view_as_graph(void) {
            return graph {
                ...
                .deduce_compare_as  = graphle::meta::deduce_as<graphle::compare_by_value<vertex>>,
                .get_vertices       = [this] { return views::all(vertices) | views::transform(util::addressof); },
                .get_out_edges      = [this] (const vertex* v) { return range_at_map_key(edges, *v) | views::transform(util::as_const) | views::transform(util::addressof) | views::edge_from(v); }
            };
        }
    };
